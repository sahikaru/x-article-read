 ...
 */
export function buildMdx(tweet: TweetData, tweetUrl: string): string {
  const date = formatDate(tweet.created_at);
...
> Author: **${tweet.author.name}** (@${tweet.author.screen_name})
> Date: ${date} · ${tweet.likes ?? 0} likes · ${tweet.views ?? 0} views
...
`;
  ...
  if (tweet.quote) {
    ...
    const qParagraphs = tweet.quote.text.split('\n').filter((l) => l.trim());
